library(Hmisc)
# Make a plot of L-test results as in RELM I: First-order results. For each 
...
for (i in 1:iModels){
  ...
  sResultFile <- paste0(sResultPath, 'probAndP_', sModelNickname, 
                        '.rds')
  vResults <- readRDS(sResultFile)
  fProb <- vResults[1]
  vObservations[i] <- fProb
  sSimulationFile <- paste0(sSimProbsPath, 'simProbs_', sModelNickname, '.rds')
...
title("L-Test", xlab = "log-likelihood")
